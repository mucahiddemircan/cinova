...
export default function SearchPage() {
  ...
  const BROWSE_CATEGORIES = [
    {
      ...
      items: [
        {
          name: t("movieCategories.popular"),
          path: "/movies/popular",
          color: "bg-indigo-900/40",
        },
        {
          name: t("movieCategories.now-playing"),
          path: "/movies/now-playing",
          color: "bg-emerald-900/40",
        },
        {
          name: t("movieCategories.upcoming"),
          path: "/movies/upcoming",
          color: "bg-rose-900/40",
        },
        {
          name: t("movieCategories.top-rated"),
    ...
    },
    {
      ...
      items: [
        {
          name: t("seriesCategories.popular"),
          path: "/series/popular",
          color: "bg-cyan-900/40",
        },
        {
          name: t("seriesCategories.on-the-air"),
          path: "/series/on-the-air",
          color: "bg-purple-900/40",
        },
        {
          name: t("seriesCategories.top-rated"),
  ...
  const { data: results, isLoading: loading } = useQuery({
    ...
    queryFn: () => contentApi.search(query, activeFilter),
    enabled: !!query.trim(),
  });
  ...
  const RelevantItem = ({ item }: { item: any }) => {
    ...
    const handleClick = () => {
      if (item.type === "movie" || item.type === "series") {
        router.push(
          getLocalizedPath(
            `/${item.type === "series" ? "series" : "movies"}/${item.id}`
          )
        );
      } else if (item.type === "person") {
        router.push(getLocalizedPath(`/people/${item.id}`));
      } else if (item.type === "profile") {
        router.push(getLocalizedPath(`/${item.title}`));
      }
    };
    ...
    return (
      <div className="relative group/card">
        <div className="flex items-center justify-between gap-3 p-2 rounded-xl bg-transparent hover:bg-white/10 transition-all cursor-pointer group/card border border-transparent">
          <div
            onClick={handleClick}
                ...
                {!isPerson && (
                  <>
                    ...
                    <span className="truncate italic text-white/40">
                      {resolveGenreNames(item.genre_ids, getGenreName, item.type)}
                    </span>
                  </>
                )}
              </div>
      ...
      return (
        <section className="mb-8">
          <h2 className="text-lg font-bold text-text-primary mb-3 flex items-center gap-2">
            {title}
            <span className="text-xs font-normal text-text-secondary opacity-40">
              ({items.length})
            </span>
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-1">
            {items.map((item) => (
              <RelevantItem key={`${item.type}-${item.id}`} item={item} />
            ))}
          </div>
        </section>
      );
    ...
    return (
      <section className="mb-10">
        <h2 className="text-xl font-bold text-text-primary mb-4 flex items-center gap-2">
          {title}
          <span className="text-sm font-normal text-text-secondary opacity-50">
            ({items.length})
          </span>
        </h2>
        <div
          className={
            isPerson === true
              ? "grid grid-cols-2 sm:grid-cols-4 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 rounded-2xl overflow-hidden"
              : `grid gap-6 grid-cols-2 sm:grid-cols-4 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6`
          }
        >
          {items.map((item) => {
            const isPersonType =
              isPerson === true || ["person", "profile"].includes(item.type);
            return (
              <div key={`${item.type}-${item.id}`}>
                {isPersonType ? (
                  <PersonCard person={item} user={user} />
                ) : (
    ...
    <div className="py-6 animate-fade-in">
      ...
      {!query.trim() ? (
        <div className="space-y-10">
          ...
          {BROWSE_CATEGORIES.map((section) => (
            <div key={section.title} className="space-y-4">
              <h3 className="text-lg font-bold text-text-secondary tracking-widest">
                {section.title}
              </h3>
              <div className="grid grid-cols-2 gap-4">
                {section.items.map((item) => (
                  ...
                  >
                    <span className="text-lg font-black text-white leading-tight relative z-10">
                      {item.name}
                    </span>
        ...
        <>
          <div className="flex flex-wrap gap-2 mb-10 overflow-x-auto pb-2 scrollbar-none">
            {FILTERS.map((f) => (
              <button
                ...
                className={`px-6 py-2 rounded-full text-sm font-semibold transition-all duration-300 border whitespace-nowrap ${
                  activeFilter === f.id
          ...
          ) : activeFilter === "all" && results && "relevant" in results ? (
            <div className="space-y-4">
              {renderGroup(
                t("search.mostRelevant"),
                results.relevant as any[],
                null,
                "list"
              )}
              {renderGroup(t("search.movies"), results.movies as any[])}
              {renderGroup(t("search.series"), results.series as any[])}
              {renderGroup(t("search.people"), results.people as any[], true)}
              {renderGroup(
                t("search.profiles"),
