/**
 * Yayın platformları sayfası bileşeni.
 *
 * İçeriğin hangi platformlarda izlenebileceğini ülke bazlı gösterir.
 */
...
import { ChevronLeft, Globe, XCircle } from "lucide-react";
import PlaceholderImage from "../components/common/PlaceholderImage";

import LoadingDots from "../components/common/LoadingDots";

const COUNTRY_CODES = ["TR", "US", "DE", "FR", "GB", "ES", "IT", "CA", "AU", "IN", "JP", "KR"];
...
export default function WatchView() {
    ...
    const [selectedCountry, setSelectedCountry] = useState(language === 'tr' ? "TR" : "US");

    useScrollRestoration(`watch_scroll_${type}_${id}`, !loading);
    ...
    if (!movie) {
        return <div className="text-center py-20 text-white">{t("watch.noSource")}</div>;
    }

    const p = movie.watch_providers?.[selectedCountry] || null;

    const hasProviders = p && (
        p.flatrate?.length > 0 ||
        p.ads?.length > 0 ||
        p.free?.length > 0 ||
        p.rent?.length > 0 ||
        ...
        <div className="animate-fade-in overflow-x-hidden">
            ...
            <div className="flex flex-col md:flex-row items-center gap-8 mb-12">
                ...
                <div className="flex-1 text-center md:text-left">
                    ...
                    <p className="text-xl md:text-2xl text-brand font-bold tracking-tight">{t("watch.watchOptions")}</p>
                </div>
            </div>
            ...
            <div className="bg-bg-surface p-6 rounded-2xl border border-bg-surface-hover mb-8 flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-lg font-bold text-white leading-tight">{t("watch.changeRegion")}</h2>
                        <p className="text-xs text-text-secondary">{t("watch.regionDisclaimer")}</p>
                    </div>
                </div>
                <select
            ...
            {!hasProviders ? (
                <div className="bg-bg-surface p-12 rounded-2xl border border-bg-surface-hover text-center space-y-4">
                    ...
                    <p className="text-text-secondary font-medium">{t("watch.noProviders")}</p>
                </div>
            ) : (
                <div className="bg-bg-surface p-8 md:p-12 rounded-[2rem] border border-bg-surface-hover shadow-2xl">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                        {p.flatrate?.length > 0 && <ProviderSection title={t("watch.subscription")} items={p.flatrate} link={p.link} />}
                        {p.ads?.length > 0 && <ProviderSection title={t("watch.ads")} items={p.ads} link={p.link} />}
                        {p.free?.length > 0 && <ProviderSection title={t("watch.free")} items={p.free} link={p.link} />}
                        {p.rent?.length > 0 && <ProviderSection title={t("watch.rent")} items={p.rent} link={p.link} />}
                        {p.buy?.length > 0 && <ProviderSection title={t("watch.buy")} items={p.buy} link={p.link} />}
                    </div>
                </div>
            )}
...
function ProviderSection({ title, items, link }) {
    return (
        <div className="space-y-6">
            <h3 className="text-sm font-bold text-white opacity-80">
                {title}
            </h3>
