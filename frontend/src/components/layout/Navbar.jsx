/**
 * Ana navigasyon çubuğu bileşeni.
 *
 * Masaüstü: Logo, kategori dropdown'ları, arama alanı, profil menüsü.
 * Mobil: Hamburger menü butonu ve MobileMenu overlay bileşeni.
 */
...
export default function Navbar({ user, onLogout }) {
    ...
    const [profileMenuView, setProfileMenuView] = useState("main"); // "main" veya "language"
    ...
    const handleSearch = () => {
        if (searchTerm.trim()) {
            ...
            inputRef.current?.blur(); // Aramadan sonra odağı kaybettir (Blur)
    ...
    useEffect(() => {
        ...
        // URL'den 'q' parametresini oku ve local state'i güncelle
    ...
    useEffect(() => {
        const fetchResults = async () => {
            ...
            try {
                ...
                // "all" araması nesne döner {movies: [], series: [], people: [], profiles: [], relevant: []}
                // Biz bunları tek bir düz liste haline getirelim veya en alakalı olanları gösterelim
                const combined = [
                    ...(data.relevant || []),
                    ...(data.movies || []),
                    ...(data.series || []),
                    ...(data.people || []),
                    ...(data.profiles || [])
                ].filter((v, i, a) => a.findIndex(t => t.id === v.id && t.type === v.type) === i); // Duplicate engelle

                setSearchResults(combined.slice(0, 10)); // İlk 10 sonucu göster
                setSelectedIndex(-1);

                // Sadece input odaktayken dropdown'ı göster. 
                // URL senkronizasyonu sırasında (Focus yokken) açılmasını engeller.
    ...
    return (
        <>
            <header className="fixed top-0 left-0 right-0 z-50 bg-bg-base/90 backdrop-blur-sm border-b border-bg-surface w-full">
                <nav className="container mx-auto px-6 py-3 md:py-4 flex items-center justify-between gap-4">
                    ...
                    <div className="flex-1 flex items-center gap-8">
                        <LocalizedLink to="/" className="text-brand text-xl md:text-2xl font-black tracking-wider uppercase">
                            {t("common.brand")}
                        </LocalizedLink>
            ...
            {/* Mobil/Tablet bildirim tam ekran */}
            {user && (
                <MobileNotificationScreen
...
/**
 * Masaüstü tıklanabilir dropdown menü bileşeni.
 */
...
/**
 * YouTube tarzı seçenekler menüsü (Dil ve Yardım).
 */
function OptionsDropdown() {
    const [isOpen, setIsOpen] = useState(false);
    const [view, setView] = useState("main"); // "main" veya "language"
    const dropdownRef = useRef(null);
    ...
    useEffect(() => {
        ...
        if (isOpen) {
            ...
            setView("main"); // Kapanınca ana menüye dön
        }
        return () => document.removeEventListener("mousedown", handleClickOutside);
    }, [isOpen]);
...
/**
 * Hızlı arama sonuçları dropdown bileşeni.
 */
