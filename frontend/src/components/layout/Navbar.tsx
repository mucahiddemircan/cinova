...
// CardActions and FollowButton will be migrated later, we use any/ignore for now
...
export default function Navbar({ user, onLogout }: NavbarProps) {
  ...
  const [profileMenuView, setProfileMenuView] = useState<"main" | "language">(
    "main"
  );
  ...
  const { data: searchData, isFetching: isSearching } = useQuery({
    queryKey: ["navbar-search", debouncedSearchTerm],
    queryFn: () => contentApi.search(debouncedSearchTerm, "all"),
    enabled: debouncedSearchTerm.trim().length >= 2,
  });

  const searchResults = (() => {
    if (!searchData || debouncedSearchTerm.trim().length < 2) return [];
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-bg-base/90 backdrop-blur-sm border-b border-bg-surface w-full">
        <nav className="container mx-auto px-6 py-3 md:py-4 flex items-center justify-between gap-4">
          <div className="flex-1 flex items-center gap-8">
            <LocalizedLink
              href="/"
              className="text-brand text-xl md:text-2xl font-black tracking-wider flex items-center gap-2 hover:scale-105 transition-transform"
            >
          ...
          <div
            className="hidden md:flex flex-shrink-0 w-full max-w-md relative"
            ref={searchRef}
          >
            <div className="flex items-center bg-bg-surface hover:bg-bg-surface-hover group rounded-full overflow-hidden h-11 w-full border border-transparent focus-within:border-brand focus-within:bg-bg-base/80 transition-all">
              <input
                ref={inputRef}
                type="text"
                value={searchTerm}
          ...
          <div className="flex-1 flex items-center justify-end">
            ...
            {user ? (
                    ...
                    {profileMenuView === "main" ? (
                      <>
                        ...
                        <LocalizedLink
                          ...
                          className="flex items-center gap-3 px-5 py-2.5 text-sm text-text-primary hover:bg-white/20 transition-colors group"
                        >
                        ...
                        <button
                          ...
                          className="w-full flex items-center gap-3 px-5 py-2.5 text-sm text-text-primary hover:bg-white/20 transition-colors group cursor-pointer text-left"
                        >
                      ...
                      <>
                        ...
                        <button
                          ...
                          className={`w-full flex items-center justify-between px-5 py-2.5 text-sm transition-colors cursor-pointer ${
                            language === "tr"
              ...
              <div className="hidden lg:flex items-center space-x-6">
                ...
                <LocalizedLink
                  href="/login"
                  className="bg-brand hover:bg-brand-hover text-white px-7 py-2.5 rounded-full text-sm font-bold transition-all active:scale-95"
                >
                  {t("nav.login")}
                </LocalizedLink>
              </div>
              ...
              {user ? (
                <div className="flex items-center gap-2">
                  <NotificationBell
                    ...
                    className="w-9 h-9"
                  />
                  <LocalizedLink href={`/${user.username}`}>
                    <Avatar
                      src={user?.avatar_url}
                      alt={user?.username || "User"}
                      size="sm"
                ...
                <LocalizedLink
                  href="/login"
                  className="bg-brand hover:bg-brand-hover text-white px-5 py-2 rounded-full text-xs font-bold transition-all active:scale-95"
                >
                  {t("nav.login")}
                </LocalizedLink>
              )}
...
function CategoryDropdown({ label, items, type }: CategoryDropdownProps) {
  ...
  return (
    <div className="relative mr-2 md:mr-4 flex items-center" ref={dropdownRef}>
      <button
        ...
        className={`p-2 rounded-full transition-all cursor-pointer hover:bg-white/20 ${
          isOpen
...
function SearchResultsDropdown({
  ...
  const { t } = useLanguage();
  // Using simple genre lookup instead of useMetadata hook for now
  // In a real app we might fetch metadata or use a metadata store
              ...
              return (
                <div
                  ...
                  className={`relative group/item flex items-center ${
                    index === selectedIndex ? "bg-white/10" : "hover:bg-white/20"
                  }`}
                >
                  <LocalizedLink
                    href={path}
                  ...
                  <div className="flex-shrink-0 absolute right-4 opacity-0 group-hover/item:opacity-100 transition-opacity z-20 flex items-center gap-2">
                    {item.type === "movie" || item.type === "series" ? (
                      <CardActions movie={item} user={user} variant="list" />
                    ) : (
                      <>
                        <FollowButton
                          username={item.title}
