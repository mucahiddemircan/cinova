 ...
 *
 * Masaüstü/tablet: Dropdown panel (Bell butonuna tıklanınca).
 * Mobil: Tam sayfa overlay.
 *
 * Her bildirim tıklanabilir ve ilgili sayfaya yönlendirir.
 * Tıklandığında otomatik olarak okundu işaretlenir.
 */
...
/**
 * Bildirim tipine göre ikon döner.
 */
function NotificationIcon({ type, size = 14 }) {
    const iconProps = { size, strokeWidth: 2.2, className: "text-white", fill: "currentColor" };
    switch (type) {
        case "follow":
...
/**
 * Bildirim tipine göre yönlendirme URL'si üretir.
 */
...
/**
 * Bildirim mesaj metnini döner (kullanıcı adı hariç kısım).
 */
function getNotificationText(type, t) {
    switch (type) {
        case "follow": return t("notifications.types.follow");
        case "comment_reply": return t("notifications.types.commentReply");
        case "comment_like": return t("notifications.types.commentLike");
            ...
            {isOpen && createPortal(
                ...
                >
                    {!notification.is_read && (
                        ...
                        >
                            <Check size={15} strokeWidth={2.5} className="text-text-secondary" />
...
/**
 * Tek bildirim satırı bileşeni.
 *
 * Layout: [İkon] [Mesaj metni · Zaman]  [···]
 */
function NotificationRow({ notification, onClick }) {
    const link = getNotificationLink(notification);
    const { markOneRead, deleteNotification } = useNotifications();
    const { t } = useLanguage();

    const navigate = useNavigate();
    const handleClick = (e) => {
        // Eğer tıklanan element bir link ise veya bir linkin içindeyse, ana yönlendirmeyi yapma
        if (e.target.closest('a') || e.target.closest('button')) return;

        // Tıklandığında okundu işaretle
    ...
    const profileLink = isAnonymous ? null : `/${notification.actor_username}`;

    return (
        <div
            onClick={handleClick}
            className={`flex items-start gap-3.5 px-5 py-3.5 transition-colors hover:bg-white/20 group/row cursor-pointer relative ${!notification.is_read ? "bg-white/5" : ""
                }`}
        >
            ...
            <div className="flex-1 min-w-0 pt-0.5">
                <p className="text-[13px] text-text-primary leading-snug font-medium break-words">
                    ...
                    {isAnonymous ? notification.message : getNotificationText(notification.type, t)}
                </p>
...
export function DesktopNotificationPanel({ isOpen, onClose, toggleRef }) {
    ...
    useEffect(() => {
        const handleClickOutside = (e) => {
            // Eğer panel kapalıysa veya tıklanan yer panelin içindeyse veya tıklanan yer toggle butonunun içindeyse bir şey yapma
            if (!isOpen) return;
            ...
            <div className="max-h-[420px] overflow-y-auto custom-scrollbar divide-y divide-white/[0.04]">
                {notifications.length === 0 ? (
                    <div className="py-16 text-center">
                        <Bell size={32} strokeWidth={1.5} className="text-white/20 mx-auto mb-3" />
                        <p className="text-text-secondary text-sm font-medium">{t("notifications.empty")}</p>
                    </div>
                ) : (
                    notifications.map((n) => (
...
export function MobileNotificationScreen({ isOpen, onClose }) {
    const { notifications, unreadCount, markAllRead } = useNotifications();
    const { t } = useLanguage();
    ...
    return createPortal(
        <div className={`fixed inset-0 z-[200] bg-black flex flex-col transition-all duration-300 ${isOpen ? "opacity-100 visible translate-y-0" : "opacity-0 invisible translate-y-4"}`}>
            {/* Header */}
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/5 shrink-0">
                <h2 className="text-lg font-bold text-text-primary">{t("notifications.title")}</h2>
...
// ────────────────────────────────────────────────
// Bell Button Component (Paylaşılan)
// ────────────────────────────────────────────────
