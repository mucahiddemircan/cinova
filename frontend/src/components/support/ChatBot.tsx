...
import { supportApi, type ChatMessage } from "@/lib/api";
...
export default function ChatBot() {
  const { t } = useLanguage();
  const [messages, setMessages] = useState<(ChatMessage & { isNew?: boolean })[]>([]);
  const [input, setInput] = useState("");
  const [loading, setLoading] = useState(false);
  const scrollContainerRef = useRef<HTMLDivElement>(null);

  // Initialize botWelcome message on client side after component mounts or t/language changes
  useEffect(() => {
    const welcomeText = t("staticPages.help.botWelcome") || `Merhaba! Ben ${BRAND_NAME} asistanıyım. Size nasıl yardımcı olabilirim?`;
    setMessages([
      { role: "assistant", content: welcomeText, isNew: false }
    ]);
  }, [t]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setMessages([...newMessages, { role: "assistant" as const, content: data.response, isNew: true }]);
    } catch (error) {
  ...
  return (
    <div className="bg-bg-surface border border-bg-surface-hover rounded-3xl overflow-hidden flex flex-col h-[550px] shadow-2xl">
          ...
          >
            <div
              className={`w-8 h-8 flex items-center justify-center shrink-0 ${
                msg.role === "user" ? "bg-bg-surface-hover rounded-full" : ""
              }`}
            >
              {msg.role === "user" ? (
      ...
      <form
        ...
        className="p-5 bg-bg-surface-hover/10 border-t border-bg-surface-hover"
      >
