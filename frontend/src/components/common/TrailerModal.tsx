...
function TrailerModal({ isOpen, onClose, trailerKey }: TrailerModalProps) {
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 md:p-8 animate-fade-in">
      ...
      <button
        onClick={onClose}
        className="fixed top-6 right-6 md:top-10 md:right-10 text-white/50 hover:text-white transition-all hover:scale-110 active:scale-95 z-[10000] p-3 bg-white/5 hover:bg-white/10 rounded-full backdrop-blur-md border border-white/10 group cursor-pointer"
        title="Kapat (ESC)"
      >
        ...
        {trailerKey ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center text-text-secondary gap-4">
            ...
            <p className="font-bold">
              Üzgünüz, bu içerik için fragman bulunamadı.
            </p>
          </div>
        )}
      </div>
