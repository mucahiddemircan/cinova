...
export default function CastView({ type, id }: CastViewProps) {
  const { t, getLocalizedPath } = useLanguage();
  const router = useRouter();
  const [activeFilter, setActiveFilter] = useState<"cast" | "crew" | null>(null);

  const toggleFilter = (filterId: "cast" | "crew") => {
  ...
  const { data: castData, isLoading: castLoading } = useQuery({
    ...
    queryFn: () => contentApi.getCast(type, id),
  });
  ...
  if (!castData || !movie) {
    return (
      <div className="text-center py-20 text-white font-medium opacity-50">
        {t("cast.noData")}
      </div>
    );
  }

  const CastMemberCard = ({ person, subtext }: { person: any; subtext?: string }) => {
    const navPath = getLocalizedPath(`/people/${person.id}`);
    ...
    return (
      ...
      >
        <Avatar src={person.profile_path} alt={person.name} size="lg" type="person" />
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-white/40 font-medium mt-0.5 truncate">
            {subtext || t("cast.title")}
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-16 px-2">
        <div className="flex items-center gap-6">
          <div className="text-center md:text-left">
            ...
            <p className="text-sm text-text-secondary font-medium mt-1 opacity-50">
              {t("cast.fullCastAndCrew")}
            </p>
          </div>
        </div>
        ...
        {(activeFilter === null || activeFilter === "cast") && (
          <div className="animate-fade-in">
            <div className="flex items-center justify-center md:justify-start gap-4 mb-8">
              <h2 className="text-lg font-bold text-white px-2 py-1.5">
                {t("cast.actors")}
              </h2>
            </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                    {members.map((person, idx) => (
                      <CastMemberCard
                        ...
                        person={person}
                        subtext={person.job || person.known_for_department}
                      />
                    ))}
                  </div>
                </div>
