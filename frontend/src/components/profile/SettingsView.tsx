...
function renderInlineFeedback(
  feedback: { message: string | null; error: string | null } | undefined
) {
...
export default function SettingsView({ user, onUserUpdate }: SettingsViewProps) {
  ...
  // Scoped feedback
  const [feedbacks, setFeedbacks] = useState<Record<string, { message: string | null; error: string | null }>>({
    username: { message: null, error: null },
    email: { message: null, error: null },
    password: { message: null, error: null },
  });

  const setSectionFeedback = (section: string, message: string | null, error: string | null) => {
    setFeedbacks((prev) => ({
      ...prev,
  ...
  const clearFeedback = (section: string | null = null) => {
    if (section) {
  ...
  const getRemainingDays = useCallback(() => {
    ...
    return Math.ceil((cooldownEnd.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
  }, [accountStatus]);
  ...
  const handleChangeUsername = async () => {
    ...
    try {
      ...
      setSectionFeedback("username", (res as any).message, null);
      ...
      setSectionFeedback("username", null, err.message || t("errors.usernameChange"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleChangeEmail = async () => {
    ...
    try {
      ...
      setSectionFeedback("email", null, err.message || t("errors.emailChange"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSetPassword = async () => {
    ...
    if (newPassword !== confirmPassword) {
      setSectionFeedback("password", null, t("validation.passwordsNotMatch"));
      return;
    }
    ...
    try {
      ...
      setSectionFeedback("password", (res as any).message, null);
      ...
      setSectionFeedback("password", null, err.message || t("errors.passwordSet"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      setSectionFeedback("password", null, err.message || t("errors.passwordChange"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const inputBase =
    "w-full bg-bg-base text-text-primary border border-bg-surface focus:border-brand rounded-lg px-4 py-3 outline-none transition-all placeholder:text-text-secondary/30";
          ...
          <section className="bg-bg-surface/30 border border-bg-surface/50 rounded-2xl overflow-hidden backdrop-blur-md shadow-2xl">
            ...
            <div className="p-6 space-y-8">
              ...
              <div className="group">
                ...
                {renderInlineFeedback(feedbacks.username)}
                {!usernameEditing ? (
                  <div className="flex flex-col sm:flex-row gap-3">
                    <div className="relative flex-1">
                      <input
                        ...
                        value={user.username}
                        disabled
                        className={`${inputBase} !bg-bg-base/30 !text-text-secondary/50 cursor-not-allowed`}
                      />
                      <CheckCircle2
                        size={16}
                        className="absolute right-4 top-1/2 -translate-y-1/2 text-green-500/50"
                      />
                    </div>
                    <button
                      onClick={() => {
                  ...
                  <div className="space-y-4 p-4 bg-bg-base/30 rounded-2xl border border-white/5">
                    <div>
                      <input
                        ...
                        className={`${inputBase} ${
                          fieldErrors.username ? inputErrorClass : ""
                        }`}
                    ...
                    {accountStatus?.has_password && (
                      <input
                        ...
                        className={inputBase}
                        placeholder={t("settings.currentPasswordSecurity")}
                      />
                    )}
                    <div className="flex gap-3 pt-2">
                      ...
                      <button
                        ...
                        className="text-text-secondary hover:text-white px-4 py-3 text-sm font-semibold transition-colors cursor-pointer"
                      >
                        {t("common.cancel")}
                      </button>
                    </div>
              ...
              <div>
                <label className="block text-text-secondary text-xs font-semibold mb-4 opacity-70">
                  {t("settings.email")}
                </label>
                {renderInlineFeedback(feedbacks.email)}
                {!emailEditing ? (
                  <div className="flex flex-col sm:flex-row gap-3">
                    <div className="relative flex-1">
                      <input
                      ...
                      <div>
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            validateField("emailNewPassword", e.target.value);
                          }}
                          className={`${inputBase} ${
                            fieldErrors.emailNewPassword ? inputErrorClass : ""
                          }`}
                        ...
                        <p className="text-[11px] text-text-secondary mt-2 ml-1 opacity-60">
                          {t("settings.googleLinkWarning")}
                        </p>
                      </div>
                    )}
          ...
          <section className="bg-bg-surface/30 border border-bg-surface/50 rounded-2xl overflow-hidden backdrop-blur-md shadow-2xl">
            ...
            <div className="p-6 space-y-8">
              <div>
                ...
                {renderInlineFeedback(feedbacks.password)}
                {passwordMode === null ? (
                  <button
                    onClick={() => {
                    ...
                    <div className="space-y-4">
                      <div className="relative">
                        <input
                          ...
                          className={`${inputBase} pr-12 ${
                            fieldErrors.newPassword ? inputErrorClass : ""
                          }`}
                          placeholder={
                            passwordMode === "set"
                              ? t("settings.setPassword")
                      ...
                      <div className="flex gap-3 pt-2">
                        <button
                          onClick={passwordMode === "set" ? handleSetPassword : handleChangePassword}
                          disabled={
                            loading ||
                            !!fieldErrors.newPassword ||
                            !newPassword ||
                            newPassword !== confirmPassword ||
                            (passwordMode === "change" && !currentPassword)
                          }
                          className="bg-brand hover:bg-brand-hover disabled:bg-bg-surface-hover disabled:text-text-secondary text-white px-8 py-3 rounded-xl font-semibold text-sm transition-all cursor-pointer"
                        >
                          {loading ? (
                  ...
                  <div className="absolute right-0 mt-2 w-48 bg-bg-surface border border-white/10 rounded-xl shadow-2xl overflow-hidden z-50 animate-slide-down py-1.5 backdrop-blur-xl">
                    {[
                      ...
                      >
                        ...
                        {language === lang.value && <Check size={16} className="text-white" />}
                      </button>
                    ))}
                  </div>
                )}
