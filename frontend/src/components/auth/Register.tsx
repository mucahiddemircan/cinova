...
export default function Register({ onLogin }: RegisterProps) {
  ...
  const validateField = async (
    ...
    } else if (name === "birthDate" && typeof value === "object") {
      if (!value.day || !value.month || !value.year) {
        err = t("auth.birthDateReq");
      } else {
        ...
        if (isNaN(d) || isNaN(y) || isInvalidDate)
          err = t("auth.birthDateInvalid");
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const isBirthDateValid = await validateField("birthDate", {
      day,
      month,
      year,
    });
    ...
    try {
      const birth_date = `${year}-${(months.indexOf(month) + 1)
        .toString()
      ...
      if (data.user && !data.session) {
        ...
        onLogin(data.user as unknown as User);
      }
    } catch {
  ...
  return (
    <div className="w-full px-4 sm:px-8 py-4 animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="space-y-6" noValidate>
            ...
            <div className="relative">
              <select
                ...
                className={`${inputClass(
                  ...
                  !month ? "text-auth-muted" : "text-text-primary"
                }`}
              >
                <option value="" disabled className="bg-bg-base text-auth-muted">
                  {t("auth.month")}
                </option>
                {months.map((m) => (
                  <option
              ...
              <ChevronDown
                size={12}
                strokeWidth={3}
                className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-auth-muted"
              />
            </div>
            <input
              type="text"
        ...
        <div>
          <label className="block text-white text-sm font-semibold mb-2 ml-1">
            {t("user.username") || t("auth.username")}
          </label>
          <input
            type="text"
            value={username}
      ...
      <div className="relative flex items-center justify-center my-8">
        <div className="border-t border-auth-muted/30 w-full" />
        <span className="bg-bg-base px-4 text-auth-muted text-xs absolute">
      ...
      <p className="mt-8 text-center text-auth-muted text-sm">
        ...
        <LocalizedLink
          href="/login"
          className="text-white hover:underline ml-1 font-bold cursor-pointer"
        >
          {t("auth.loginLink")}
        </LocalizedLink>
      </p>
