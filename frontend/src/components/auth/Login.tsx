...
export default function Login({ onLogin }: LoginProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (authError) {
      let msg: React.ReactNode = authError.message;
      ...
      // Set access token immediately to prevent race conditions during auth redirection
  ...
  const handleResendConfirmation = async (emailToResend: string) => {
    const { error: resendError } = await authApi.resendConfirmation(
      emailToResend
    );
    if (resendError) {
  ...
  return (
    <div className="w-full px-4 sm:px-8">
      ...
      <p className="mt-8 text-center text-auth-muted text-sm">
        ...
        <LocalizedLink
          href="/register"
          className="text-white hover:underline ml-1 font-medium cursor-pointer"
        >
          {t("auth.registerLink")}
        </LocalizedLink>
      </p>
