...
export default function CardActions({ movie, user, onStatusChange, variant = "card" }) {
    ...
    const handleListSelect = async (newStatus) => {
        ...
        // If already in this list, remove it (Toggle logic)
    ...
    const calculatePosition = (triggerRef, menuWidth, estimatedHeight = 300) => {
        ...
        // By default align with left edge of button (opens to right)
        let left = rect.left;

        // Right overflow check: If opening to right goes offscreen, open to left
        if (left + menuWidth > viewportWidth - 15) {
            left = rect.right - menuWidth;
        }

        // If still overflowing from left (does not fit screen), snap to left
        if (left < 10) {
            left = 10;
        }

        // Vertical overflow check - If it doesn't fit downwards, configure to open upwards
    ...
    // if (!user) return null; // Unauth users can also see buttons and will get a warning when clicked.
