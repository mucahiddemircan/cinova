 ...
 * Manages replies, editing and deleting operations.
 */
...
export default function CommentItem({ comment, user, onUpdate }) {
    const { t } = useLanguage();
    ...
    const [interaction, setInteraction] = useState(comment.user_interaction);
    ...
    const handleReply = async (content, isSpoiler) => {
        try {
    ...
    const handleUpdate = async (content, isSpoiler) => {
        try {
    ...
    // Hash-based highlight: highlight the comment matching #comment-{id} in the URL
    useEffect(() => {
        ...
        if (targetId === comment.id) {
            // This comment is target — apply scroll and highlight
            const timeout = setTimeout(() => {
                if (commentRef.current) {
                    commentRef.current.scrollIntoView({ behavior: "smooth", block: "center" });
                    setIsHighlighted(true);
                    const animTimeout = setTimeout(() => setIsHighlighted(false), 2600);
                    return () => clearTimeout(animTimeout);
                }
            }, 300);
            return () => clearTimeout(timeout);
        } else if (repliesCount > 0 && !showReplies) {
            // Hash is not for this comment but might belong to one of our replies — open automatically
            setShowReplies(true);
        }
    }, [location.hash, comment.id, repliesCount]);
    ...
    return (
        <>
            <div
                ...
                className={`flex gap-4 items-start mb-6 group/item pt-2 scroll-mt-24 transition-colors ${isHighlighted ? "comment-highlight" : ""}`}
            >
                <Link to={`/${comment.username}`} className="shrink-0">
                    <Avatar
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-4 mb-1 pr-2">
                        <div className="flex items-center gap-2">
                            <Link to={`/${comment.username}`} className="text-sm font-bold text-white hover:underline cursor-pointer tracking-tight">
                                @{comment.username}
                            </Link>
                            <span className="text-[11px] text-text-secondary flex items-center gap-1.5">
                                {formatTimeAgo(comment.created_at, t)}
                                {comment.is_edited && <span className="opacity-40 font-medium">({t("comments.edited")})</span>}
                            </span>
                        </div>
                            ...
                            {isMenuOpen && (
                                <div className="absolute right-0 top-full mt-1 bg-bg-surface border border-white/5 rounded-xl shadow-2xl overflow-hidden min-w-[140px] z-[60] animate-fade-in shadow-black/80">
                                    {isOwner ? (
                                        ...
                                        <button
                                            onClick={() => { setIsReplying(true); setIsMenuOpen(false); }}
                                            className="w-full px-4 py-3 text-sm text-text-secondary hover:text-white hover:bg-white/5 transition-colors cursor-pointer flex items-center gap-3"
                                        >
                        ...
                        <>
                            ...
                            <div className="flex items-center gap-1 -ml-2 mb-2">
                                <button
                                    ...
                                    className={`flex items-center gap-1.5 px-2 py-1.5 rounded-full hover:bg-white/5 transition-all text-[12px] font-bold cursor-pointer ${interaction === "like" ? "text-white" : "text-white"}`}
                                >
                                    <ThumbsUp size={16} strokeWidth={2} fill={interaction === "like" ? "currentColor" : "none"} />
                                    {likesCount > 0 && likesCount}
                                </button>

                                <button
                                    onClick={() => handleInteract("dislike")}
                                    className={`flex items-center gap-1.5 px-2 py-1.5 rounded-full hover:bg-white/5 transition-all text-[12px] font-bold cursor-pointer ${interaction === "dislike" ? "text-white" : "text-white"}`}
                                >
                                    <ThumbsDown size={16} strokeWidth={2} fill={interaction === "dislike" ? "currentColor" : "none"} />
                    ...
                    {repliesCount > 0 && (
                        ...
                        >
                            <ChevronDown size={16} strokeWidth={2} className={showReplies ? "rotate-180 transition-transform" : "transition-transform"} />
                    ...
                    {showReplies && (
                        <div className="mt-4 pl-0 md:pl-6 border-l border-white/5 ml-5">
                            {repliesLoading ? (
                                ...
                                replies.map(reply => (
                                    <CommentItem key={reply.id} comment={reply} user={user} onUpdate={loadReplies} />
                                ))
                            )}
                        </div>
                    )}
            ...
            {showDeleteModal && createPortal(
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-bg-surface border border-white/10 rounded-2xl w-full max-w-[400px] overflow-hidden shadow-2xl animate-scale-up">
                        <div className="p-6">
                            <h3 className="text-lg font-bold text-white mb-2">{t("comments.deleteTitle")}</h3>
