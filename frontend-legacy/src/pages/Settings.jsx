...
export default function Settings({ user, onUserUpdate }) {
    ...
    // Scoped feedback (Directive 6 Fix)
    ...
    const [emailPassword, setEmailPassword] = useState(""); // Current password
    const [emailNewPassword, setEmailNewPassword] = useState(""); // Initial password for Google users
    ...
    // Validation Function (Adapted from Register.jsx)
    ...
    const handleChangeEmail = async () => {
        ...
        // Google-only user must set a password
        ...
        try {
            ...
            // Backend verification and password setting (if needed)
    ...
    const handleSetPassword = async () => {
        ...
        try {
            ...
            const status = await accountApi.getStatus();
            setAccountStatus(status);
            // Refresh user data since we upgraded to Hybrid
            if (onUserUpdate) {
                const me = await authApi.getMe();
                onUserUpdate(me);
            }
    ...
    return (
        <div className="max-w-4xl mx-auto py-8 px-4 animate-fade-in">
            ...
            {/* Global Feedback removed, will be added inline */}
                            ...
                            <div>
                                ...
                                {!emailEditing ? (
                                    ...
                                    <div className="space-y-4 p-5 bg-bg-base/30 rounded-2xl border border-white/5">
                                        {/* If Google or Hybrid, show connection break warning */}
                                        ...
                                        {/* For Classic or Hybrid (those with password), ask current password */}
                                        {accountStatus?.has_password ? (
                                            ...
                                            /* Let only Google-only users set a new password */
                                            <div>
                                                <input
                                                    type="password"
